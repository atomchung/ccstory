...
ccstory reads narratives natively from `~/.claude/projects/*/`*.jsonl —
zero subprocess, zero `claude -p`, zero cost.
"""
...
def _run_init(argv: list[str], console: Console) -> int:
    ...
    p = argparse.ArgumentParser(
        prog="ccstory init",
        description="Scan recent sessions and write a default config.toml "
                    "with rule-based bucket suggestions. No LLM call.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "      Rule-based bucket suggestion from recent project names.\n"
            "      Writes ~/.ccstory/config.toml. No LLM call.\n"
            "  ccstory trend [--weeks N | --months N]\n"
            "      Per-bucket sparklines + burn-% over N periods.\n"
            "\n"
            "Examples:\n"
            "  ccstory week                  # last 7 days, native narratives\n"
            "  ccstory 2026-04               # specific month\n"
            "  ccstory trend --months 6      # 6-month sparkline view\n"
            "  ccstory init -y               # auto-categorize (no prompt)\n"
            "\n"
            "Narrative source (per session, pure jsonl read):\n"
            "  Tier 1: built-in /recap output (system/away_summary)\n"
            "  Tier 2: first user message (always-on fallback)\n"
            "ccstory never invokes `claude -p` — zero subprocess, zero cost."
        ),
