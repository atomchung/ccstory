...
Reframe the period around the user's WORK THEMES, NOT a category-by-category log. Merge the categories into 2-4 work themes reflecting what the user worked toward (fold tool-building categories into one "build" theme; keep investing research its own theme). Lead with the theme that took the most time.

For EACH theme write a block with TWO parts:
...
2) Then 1-3 bullet points (each line starts with "- "): one concrete thing done, phrased as an outcome, not a pile of technical nouns. Use the minimum that covers the theme — most themes need 1-2 bullets; write a 3rd only when there's a genuinely distinct third outcome, never by splitting one outcome into parts to hit the cap.
...
Rules:
- 2-4 themes total. Header = the key concrete win + its payoff, specific to THIS week. Bullets carry the supporting detail.
- 1-3 bullets per theme — don't pad to 3 by default; a theme with one clear outcome gets one bullet. NO issue/PR/commit numbers anywhere (no "#162", no "closes #31"); those live in a separate table. Name qualitative outcomes.
- Ground every claim in the summaries. Never invent a version or outcome. If a theme has no clear "before", just state the concrete win and its payoff; do not fabricate a before/after.
...
Output only the theme blocks (bold header + bullet lines) — no preamble, no quotes, no fences."""
...
def synthesize_overall_for_period(
    ...
    """Synthesize the overall work-theme narrative for the whole period.
